...
const NuevoPago = ({prop,selectedFactura,setOpen}) => {
  const { facturasDeProveedores,setFacturasDeProveedores,setGastos,gastos,setPagos,pagos,ingresos,setIngresos,facturas,setFacturas,cobranzas,setCobranzas,ordenarFechas,bancos,setBancos } = useContext(AppContext);
  ...
  async function sendDatos (){
    if(cobranzaPorFactura === true ){
      ...
      const data = {
        ...
        concepto: "pago de factura de proveedores",
        valor: null,
        impuesto: null,
        cantidad: null,
        observaciones: null,
      ...
      console.log(data)
      try{
        await axios.post('http://localhost:3000/api/pagos',data)
        
        const updateFacturas = facturasDeProveedores.map((item)=>{
          if(item.idFactura === facturaSeleccionada.idFactura){
            return {...item,estado:"Pagada",montoPagado:facturaSeleccionada.total,montoPendiente:0}
          }
          return item
        })

        const updatePagos = ordenarFechas([data,...pagos])
        setPagos(updatePagos)
        setFacturasDeProveedores(updateFacturas)
        setGastos(gastos + data.total)
        console.log(`aumentando el total de los gastos ${gastos + data.total}`)
...

      {
        ...
        <>
          <div style={{marginTop:30,width:"100%",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:20}}>
            <div style={{border:"1px solid black", padding:"20px 60px",borderRadius:10}}>
              ...
              <div onClick={()=>{setCobranzaPorFactura(true)}} style={{cursor:"pointer",display:"flex",alignItems:"center",gap:10,marginTop:20}}>
                ...
                <div>Pago asociado a una factura de proveedores</div>
              </div>
              <div onClick={()=>{setCobranzaPorFactura(false)}} style={{cursor:"pointer",display:"flex",alignItems:"center",gap:10,marginTop:20}}>
                ...
                <div>Pago NO asociado a una factura de proveedores</div>
              </div>
            </div>
          </div>
        </>
